...
from MNCRSApp.forms import ValidateFrm
...
def Investigator_view_cmplaints(request):
    ...
    reports = Crime_Reporting.objects.select_related('Reporter_ID').filter(Investigator_ID=request.session['ID'])
    encrypted_name=reports[0].Reporter_ID.Nick_Name
    print("name:",encrypted_name)
    plain_name = ""
    for c in encrypted_name:
        x = ord(c)
        x = x - 1
        c2 = chr(x)
        plain_name = plain_name + c2
    return render(request,"MNCSApp/Investigator_Crime_Events.html",{'Reports':reports, 'Reporter':plain_name} )
...
def error(request):
    return render(request, "MNCSAdministration/error.html")
...
def ManageReporters(request):
    reports = Crime_Reporting.objects.select_related('Reporter_ID').filter(Status='Fake Lead')
    # print(reports,"count:",reports.count())
    # for i in range(0,reports.count()):
    #     encrypted_name[i]=reports[i].Reporter_ID.Nick_Name
    #     print(encrypted_name[i])
    #     status.append(reports[i].Reporter_ID.Blocked)
    #     # reporters = Registration.objects.all()
    #     # encrypted_name=reporters[0].Nick_Name
       
    #     plain_name[i] = ""
    #     for c in encrypted_name[i]:
    #         x = ord(c)
    #         x = x - 1
    #         c2 = chr(x) 
    #         plain_name[i] = plain_name[i] + c2
    FKcount=Crime_Reporting.objects.filter(Status='Fake Lead').values('Reporter_ID').annotate(count=Count('Reporter_ID'))
    
    return render(request, "MNCSAdministration/manage_reporters.html", { 'reporters': reports,'Fake':FKcount})
...
def Allot_Investigators(request, id):
    if request.method == "POST":
        ...
        rports = Crime_Reporting.objects.get(id=id)
        rports.Investigator_ID=investigator
        rports.save()
...
def delFeedback(request,id):
    Feed=Feedback_form.objects.get(id=id)
    Feed.delete()
